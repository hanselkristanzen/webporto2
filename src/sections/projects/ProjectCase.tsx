...
import { StairsLifeVisual } from "./StairsLifeVisual";
  ...
  return (
    <article className={styles.case} aria-labelledby={`project-${project.slug}`}>
        ...
        {project.link ? (
          <Reveal delay={180}>
            <div className={styles.ctaRow}>
              <MagneticButton href={project.link} target="_blank" rel="noreferrer noopener">
                Visit Live Site
              </MagneticButton>
            </div>
          </Reveal>
      ...
      <Reveal variant="scale" delay={80} as="div" className={styles.visualSlot}>
        <div ref={tiltRef} className={styles.visualTilt} data-cursor="view">
          <span className={styles.visualLabel}>Preview / Concept Diagram</span>
          <StairsLifeVisual />
        </div>
      </Reveal>
    </article>
